...
test('public package is a standard DSH bundle with a unique identity', async () => {
  ...
  assert.ok(manifest.dsh.client.inject.includes('@deepseek-ai/dsh-client-store'))
  ...
  assert.deepEqual(compatibility.previews, ['0.1.2-alpha.1'])
...
test('release workflows reconcile an existing npm artifact by package contents', async () => {
  ...
  assert.match(workflow, /npm publish "\$package" --access public --provenance --tag "\$NPM_TAG"/)
  assert.match(workflow, /gh release create[\s\S]*"\$\{release_flags\[@\]\}"/)
