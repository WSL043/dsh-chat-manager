...
test('documentation pins the replacement slot, release asset, and second confirmation', async () => {
  ...
  for (const document of [chinese, english, agents]) {
    assert.match(document, /@deepseek-ai\/dsh-client-ui-workspace@https:\/\/github\.com\/WSL043\/dsh-session-delete\/releases\/download\/v0\.1\.9\/dsh-session-delete\.tgz/)
  }
  ...
  assert.match(chinese, /raw\.githubusercontent\.com\/WSL043\/dsh-session-delete\/v0\.1\.9\/AGENTS\.md/)
  assert.match(english, /raw\.githubusercontent\.com\/WSL043\/dsh-session-delete\/v0\.1\.9\/AGENTS\.md/)
  ...
  for (const document of [chinese, english]) {
    assert.match(document, /releases\/download\/v0\.1\.9/u)
