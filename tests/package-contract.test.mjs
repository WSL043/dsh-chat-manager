...
test('documentation uses the standard one-command bundle lifecycle and second confirmation', async () => {
  ...
  assert.match(chinese, new RegExp(`raw\\.githubusercontent\\.com/WSL043/dsh-native-session-manager/v${releaseVersion}/docs/assets/confirm-delete\\.png`))
  assert.match(chinese, new RegExp(`raw\\.githubusercontent\\.com/WSL043/dsh-native-session-manager/v${releaseVersion}/docs/assets/archive-manager\\.png`))
  assert.match(english, /second confirmation/i)
  assert.match(english, /permanent deletion cannot be undone/i)
  assert.match(english, new RegExp(`raw\\.githubusercontent\\.com/WSL043/dsh-native-session-manager/v${releaseVersion}/docs/assets/confirm-delete\\.en\\.png`))
  assert.match(english, new RegExp(`raw\\.githubusercontent\\.com/WSL043/dsh-native-session-manager/v${releaseVersion}/docs/assets/archive-manager\\.en\\.png`))
  ...
  assert.match(chinese, /便携版[\s\S]*Windows 安装版/)
  assert.match(english, /DSH-Portable/)
  assert.match(english, /portable edition[\s\S]*Windows installer edition/i)
