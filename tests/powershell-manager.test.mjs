...
const packageUrl = 'https://github.com/WSL043/dsh-session-delete/releases/download/v0.1.6/dsh-session-delete.tgz'
...
if (action === 'list') {
  ...
  for (const [name, spec] of Object.entries(deps)) {
    output[name] = { version: String(spec).includes('dsh-session-delete') ? '0.1.6' : String(spec).replace(/^[^0-9]*/, '') }
  }
...
process.exit(4)
`)
  return { fixture, portable, profileFile, commandRoot, dshBin, node: join(nodeDir, 'node.exe') }
}
...
windowsTest('manager never starts or stops DSH and publishes immutable release URLs', async () => {
  ...
  assert.match(script, /releases\/download\/v0\.1\.6\/dsh-session-delete\.tgz/)
  assert.match(script, /dsh-session-delete\.ps1\.sha256/)
  assert.match(script, /dsh-session-delete\.tgz\.sha256/)
