...
const packageUrl = 'https://github.com/WSL043/dsh-session-delete/releases/download/v0.1.8/dsh-session-delete.tgz'
...
if (action === 'list') {
  ...
  for (const [name, spec] of Object.entries(deps)) {
    output[name] = { version: String(spec).includes('dsh-session-delete') ? '0.1.8' : String(spec).replace(/^[^0-9]*/, '') }
  }
...
if (action === 'add') {
  ...
  if (name === '${packageName}' && String(spec).includes('dsh-session-delete')) {
    ...
    writeFileSync(linked, JSON.stringify({ name: 'dsh-session-delete', version: '0.1.8' }))
  }
...
windowsTest('manager never starts or stops DSH and publishes immutable release URLs', async () => {
  ...
  assert.match(script, /releases\/download\/v0\.1\.8\/dsh-session-delete\.tgz/)
