...
- Use the fixed v1.0.1 Release assets below; never substitute a moving branch or an unreviewed URL.
...
The v1.0.1 package is a standard DSH bundle with a `dsh.bundle` profile patch. Its exact package spec is:

```text
dsh-native-session-delete@https://github.com/WSL043/dsh-session-delete/releases/download/v1.0.1/dsh-native-session-delete.tgz
```
...
approach is not the v1.0.1 contract.
...
```powershell
.\dsh.exe plugin --profile web add "dsh-native-session-delete@https://github.com/WSL043/dsh-session-delete/releases/download/v1.0.1/dsh-native-session-delete.tgz"
```

Use the same `add` command to update or repair. On Windows, `install.ps1` from the same fixed Release may
be used only as a thin locator and launcher. It checks bounded known locations and invokes the exact command
...
2. Its direct package spec is the fixed v1.0.1 Release URL above.
