...
- Use the fixed v1.3.5 package below; never substitute a moving branch or an unreviewed source.
...
The v1.3.5 package is a standard DSH bundle with a `dsh.bundle` profile patch. Its exact package spec is:

```text
dsh-chat-manager@1.3.5
```
...
approach is not the v1.3.5 contract. The product is shown to users as **DSH Chat Manager**.
...
2. Its direct package spec is the fixed `dsh-chat-manager@1.3.5` npm version above.
