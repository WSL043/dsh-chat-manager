...
- Use the fixed v1.5.1 package below; never substitute a moving branch or an unreviewed source.
...
The v1.5.1 package is a standard DSH bundle with a `dsh.bundle` profile patch. Its exact package spec is:
...
approach is not the v1.5.1 contract. The product is shown to users as **DSH Chat Manager**.
...
2. Its direct package spec is the fixed `dsh-chat-manager@1.5.1` npm version above.
