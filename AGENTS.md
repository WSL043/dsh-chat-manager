...
The fixed v0.1.8 package spec is:

```text
@deepseek-ai/dsh-client-ui-workspace@https://github.com/WSL043/dsh-session-delete/releases/download/v0.1.8/dsh-session-delete.tgz
```
...
`.sha256` sidecar from the same immutable v0.1.8 Release, verify the script,
...
```powershell
.\dsh.exe plugin --profile web add "@deepseek-ai/dsh-client-ui-workspace@https://github.com/WSL043/dsh-session-delete/releases/download/v0.1.8/dsh-session-delete.tgz"
```

Use the same command for v0.1.8 update or repair. Do not restart DSH
automatically.
...
2. The direct dependency spec is the fixed v0.1.8 Release URL.
3. The installed alias directory
   `node_modules/@deepseek-ai/dsh-client-ui-workspace/package.json` reports
   `name: "dsh-session-delete"` and `version: "0.1.8"`.
