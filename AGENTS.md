...
- Use the fixed v1.2.2 package below; never substitute a moving branch or an unreviewed source.
...
The v1.2.2 package is a standard DSH bundle with a `dsh.bundle` profile patch. Its exact package spec is:
...
approach is not the v1.2.2 contract. The product is shown to users as **DSH Chat Manager**.
...
2. Its direct package spec is the fixed `dsh-chat-manager@1.2.2` npm version above.
