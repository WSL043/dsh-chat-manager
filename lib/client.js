	...
	factory: (require) => {
		...
		const copy = {
			zh: {
				tab: "会话恢复",
				title: "已归档会话",
				hint: "这里只补充官方界面暂时缺少的恢复入口。不会搜索内容，也不会删除会话。",
				empty: "没有已归档会话。",
				restore: "恢复",
				restoring: "正在恢复…",
				failed: "恢复失败，请稍后重试。"
			...
			en: {
				...
				hint: "This only fills the missing restore entry. It does not search or delete session content.",
		...
		function SessionRecoverySettingsTab({ ctx }) {
				...
				try {
					const response = await fetch("/plugins/dsh-chat-manager/restore", {
						...
						headers: {
							...
							"x-dsh-chat-manager-action": "restore-session"
						},
			...
			return React.createElement("section", {
				style: styles.root,
				"aria-label": t.title
			}, React.createElement("h2", { style: styles.heading }, t.title), React.createElement("p", { style: styles.hint }, t.hint), error && React.createElement("p", {
				role: "alert",
				style: styles.error
			}, error), archivedSessionIds.length === 0 ? React.createElement("p", { style: styles.empty }, t.empty) : React.createElement("div", { style: styles.list }, archivedSessionIds.map((sessionId) => {
				const summary = sessions.byId?.[sessionId];
				const title = String(summary?.displayTitle || summary?.title || sessionId);
				return React.createElement("div", {
					key: sessionId,
					style: styles.row
				}, React.createElement("div", { style: styles.text }, React.createElement("div", {
					style: styles.name,
					title
				}, title), title === sessionId ? null : React.createElement("div", { style: styles.id }, sessionId)), React.createElement("button", {
					type: "button",
					style: styles.button,
					disabled: busy !== "",
					onClick: () => restore(sessionId)
				}, busy === sessionId ? t.restoring : t.restore));
			})));
		}
		const styles = {
			root: {
			...
			button: {
				...
				background: "var(--dsw-alias-interactive-bg-default)"
			}
		};
		...
		function apply(ctx) {
			ctx.slots.inject("settings.plugins.tab", () => ctx.slots.register({
				...
				id: "session-recovery",
				order: 70,
				label: () => copy[languageOf(ctx)].tab
			}, () => React.createElement(SessionRecoverySettingsTab, { ctx })));
		}
		//#endregion
		exports.SessionRecoverySettingsTab = SessionRecoverySettingsTab;
		exports.apply = apply;
