...
> **请在使用前自行备份。点击“永久删除”后没有回收站、撤销或官方恢复流程。**

- 操作者必须确认自己有权处置目标会话，并自行核对目标、备份需求以及组织或法律要求的
  数据留存义务。作者和维护者不会替你判断某个会话是否应当删除。
- 当前版本只适配 DeepSeek Harness `0.1.0-rc.6` 的默认逐会话 JSONL 存储，并会替换
  原生 workspace 包的依赖位。DSH 升级、自定义 profile 或其他替换同一依赖位的插件
  可能与本插件不兼容；升级前应先卸载或重新验证。
- 本插件只删除经过校验的目标会话独占目录，不保证清除索引、缓存、附件、日志、备份、
  同步副本或其他插件保存的数据，因此不能把它当作完整的隐私擦除或合规销毁工具。
- 误选目标、未备份、兼容性变化、中断、配置冲突以及使用或无法使用本插件造成的数据
  丢失、服务中断或其他损失，由操作者自行承担风险。
- 本软件按 [MIT 许可证](LICENSE)“按原样”提供，不作任何明示或默示保证。在适用法律
  允许的最大范围内，作者和维护者不对使用或无法使用本软件引起的损害承担责任；法律
  不允许排除或限制的责任不受本说明影响。
