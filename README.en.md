> The current preview targets DSH 0.1.7-rc.1 through official session-menu extension slots. Archive search, unarchive and red permanent-delete actions appear in Settings. Compatible Portable builds provide the sidebar archive shortcut. Older cores require their corresponding plugin releases.
...
**Match the plugin to the core:** use `1.5.2-beta.4` for DSH `0.1.7-rc.1`, `1.5.2-beta.3` for `0.1.7-alpha.2`, `1.5.1` for `0.1.7-alpha.1`, or `1.4.0-beta.3` for `0.1.6-alpha.2`. Do not paste a complete `dsh plugin ...` command into the package field or treat the repository's `main` branch as a qualified release package.
...
For Agent installation of the preview, use the fixed-version [AGENTS.md](https://raw.githubusercontent.com/WSL043/dsh-chat-manager/v1.5.2-beta.4/AGENTS.md).
...
<!-- dsh-compatibility -->
The installation examples target DSH `0.1.7-rc.1` with plugin `1.5.2-beta.4`; use the matching version above for other cores.
<!-- /dsh-compatibility -->
...
Install the target npm version with the same standard DSH command. For DSH `0.1.7-rc.1`:
