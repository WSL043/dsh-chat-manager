...
$ReleaseApi = 'https://api.github.com/repos/WSL043/dsh-session-delete/releases/latest'
$ReleaseBase = 'https://github.com/WSL043/dsh-session-delete/releases/download'
...
$Messages = @{
    'zh-CN' = @{
        Banner = 'DSH Session Delete 安装助手'
        ...
        Selected = '目标'
        CheckingState = '正在检查是否已经安装会话删除插件...'
        Install = '安装'
        Update = '更新'
        Operation = '操作'
        CurrentVersion = '当前版本'
        ...
        NextUpdate = '以后更新可以直接运行：dsh-session-delete update'
        Failure = '操作没有完成。'
    ...
    'en-US' = @{
        Banner = 'DSH Session Delete Setup'
        ...
        Selected = 'Target'
        CheckingState = 'Checking whether Session Delete is already installed...'
        Install = 'Install'
        Update = 'Update'
        Operation = 'Operation'
        ...
        NextUpdate = 'For future updates, run: dsh-session-delete update'
...
function Get-InstalledPackageStatus {
    ...
    try {
        ...
        throw "The selected profile package.json is unreadable: $profileManifest"
    }

    $dependenciesProperty = $manifest.PSObject.Properties['dependencies']
    $dependencies = if ($null -ne $dependenciesProperty) { $dependenciesProperty.Value } else { $null }
    $current = if ($null -ne $dependencies) { $dependencies.PSObject.Properties[$PackageName] } else { $null }
    $legacy = if ($null -ne $dependencies) { $dependencies.PSObject.Properties[$LegacyPackageName] } else { $null }
    $currentSpec = if ($null -ne $current) { [string] $current.Value } else { $null }
    $currentVersion = if ($currentSpec -match '(?i)/releases/download/v([^/]+)/dsh-session-delete\.tgz$') {
        $Matches[1]
    } else {
        $null
    }
    ...
    return [pscustomobject]@{
        HasCurrent = $null -ne $current -and $currentSpec -match '(?i)dsh-session-delete'
...
function Get-VerifiedManager {
    ...
    $releaseResponse = Invoke-WebRequest -UseBasicParsing -Uri $releaseUri -Headers @{
        ...
        Pragma = 'no-cache'
        'User-Agent' = 'dsh-session-delete-setup'
    }
    try {
    ...
    $stage = Join-Path ([System.IO.Path]::GetTempPath()) ('dsh-session-delete-setup-' + [guid]::NewGuid().ToString('N'))
    $manager = Join-Path $stage 'dsh-session-delete.ps1'
    $checksumFile = Join-Path $stage 'dsh-session-delete.ps1.sha256'
    New-Item -ItemType Directory -Path $stage | Out-Null
    try {
        $assetBase = "$ReleaseBase/$tag"
        Invoke-WebRequest -UseBasicParsing -Uri "$assetBase/dsh-session-delete.ps1" -OutFile $manager
...
function Invoke-ManagerProcess {
    ...
    if ($Target.Mode -eq 'portable') {
        ...
        $managerArguments += @(
            '-DshExecutable', $Target.Executable,
            '-NodeExecutable', $Target.Node,
            '-DshHome', $Target.DshHome
        )
    }
    if ($NoModifyPath) { $managerArguments += '-NoModifyPath' }
...
function Invoke-Setup {
    ...
    $log = Join-Path ([System.IO.Path]::GetTempPath()) ('dsh-session-delete-setup-log-' + [guid]::NewGuid().ToString('N') + '.txt')
    try {
